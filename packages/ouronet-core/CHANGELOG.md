...
- **Rotate Governor** — Pact function `C_RotateGovernor` exists on chain (Smart Account `Σ.` only) but no UI surface ships it yet, so no builder is added in this release. Will land in a later version once `@stoachain/ouronet-codex` consumes it (post-OuronetUI-Phase-9-migration per the modular-codex spec's locked decision).
