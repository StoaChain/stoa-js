...
**MINOR (atomic-pair alignment with `@stoachain/stoa-core@4.5.0`).** Released 2026-09-15. NO code changes in this package; the version is bumped solely to hold the atomic-pair invariant. The 4.5.0 change — `KadenaWalletBuilder.createWalletPairFromDalosBitString` + the new `"stoic"` `SeedType` for the Stoic Chainweb derivation path — lives entirely in `@stoachain/stoa-core`. Vendor source remains byte-identical to upstream `@kadena/{client,cryptography-utils,types,hd-wallet}` at 1.18.3 / 0.4.4 / 0.7.0 / 0.6.2. Functionally identical to 4.4.0; consumers may pin to either interchangeably.
