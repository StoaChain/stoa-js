...
**`4.1.1` — 2026-05-08** — **PATCH (atomic with `@stoachain/kadena-stoic-legacy@4.1.1` + `@stoachain/ouronet-core@4.1.1`).** Closes audit findings: F-SEC-006 (`KadenaWallet` secret redaction via `toJSON` + `inspect.custom`), F-SEC-009 (typed `MnemonicMismatchError`), F-BUG-008 (Σ-prefix smart-account guard with `SmartAccountAuthError`), F-API-008 (`SeedType` dedup re-export), F-API-012 (`SigningError` ES2022 `Error.cause`), F-API-013 (`firstSignableButUnsatisfied` required field). Adds 4 new typed error classes (`MnemonicMismatchError`, `SmartAccountAuthError`, and two cross-package errors from ouronet-core: `KadenaShapeError`, `CodexUnknownFieldError`). **~640 specs pass.**
