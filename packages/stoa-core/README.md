...
**`4.3.7` on public npmjs** — **PATCH (dependency rename, no functional change; atomic pair with `@stoachain/kadena-stoic-legacy@4.3.7`).** Released 2026-07-22. The DALOS dependency moved scope: `@stoachain/dalos-crypto@4.0.3` → `@ouronet/dalos-crypto@4.0.4` — the same code republished under the organisation that owns it. The `./dalos` subpath re-exports exactly what it did before. Ship this if you are on 4.3.6: a published tarball's dependencies cannot be edited, so 4.3.6 keeps resolving the deprecated `@stoachain/dalos-crypto` forever. **737 specs pass.**

**`4.3.6` on public npmjs** — **PATCH (auto-gas-limit floor fix; atomic-triplet with `@stoachain/kadena-stoic-legacy@4.3.6` + `@stoachain/ouronet-core@4.3.6` — those two functionally identical to 4.3.5).** Released 2026-06-11. `calculateAutoGasLimit` now floors at `MIN_AUTO_GAS_LIMIT` (1,000) with graduated low-end multipliers — a dirty-read under-reports a *signed* tx's real cost (a trivial call dirty-read at 8 gas but needed 105+ on-chain), so `calc(8)=16` was starving txs (`Gas limit "16" exceeded`). Gas limit is a ceiling (fee charged on gas used), so the floor is free safety; the `0 → 0` no-sim sentinel is preserved. **653 specs pass.**
...
**v4.3.7** — DALOS dependency rename, no functional change (atomic with `@stoachain/kadena-stoic-legacy@4.3.7`). Released 2026-07-22. `@stoachain/dalos-crypto@4.0.3` → `@ouronet/dalos-crypto@4.0.4` — same code, new scope. Upgrade from 4.3.6 to stop resolving the deprecated name transitively. **737 specs pass.**

**v4.3.6** — auto-gas-limit floor fix (atomic with `@stoachain/kadena-stoic-legacy@4.3.6` + `@stoachain/ouronet-core@4.3.6`, both functionally identical to v4.3.5). Released 2026-06-11. `calculateAutoGasLimit` floors at `MIN_AUTO_GAS_LIMIT` (1,000) + graduated low-end multipliers, since a dirty-read under-reports a signed tx's real cost (8 reported vs 105+ needed) and was starving tiny txs. `0 → 0` sentinel preserved. **653 specs pass.**

**v4.3.5** — atomic-triplet alignment bump (atomic with `@stoachain/kadena-stoic-legacy@4.3.5` + `@stoachain/ouronet-core@4.3.5`). Released 2026-06-10. NO code changes; the v4.3.5 surface (Smart Account deploy builder + keyset-ref-aware activation + T1 IGNIS-balance fix) lives in `@stoachain/ouronet-core`. Functionally identical to v4.3.4. **653 specs pass.**
