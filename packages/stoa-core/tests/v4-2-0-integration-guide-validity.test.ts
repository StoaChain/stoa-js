...
describe("REQ-31..34: INTEGRATION-GUIDE.md doc validity", () => {
  ...
  describe("Subpath imports cited in the doc resolve via dynamic import (LC-8-D)", () => {
    const subpaths = [
      ...
      "@ouronet/ouronet-core/codex",
      "@ouronet/ouronet-core/pact",
      "@ouronet/ouronet-core/interactions",
      "@ouronet/ouronet-core/interactions/dexFunctions",
      "@ouronet/ouronet-core/interactions/ouroFunctions",
      "@ouronet/ouronet-core/interactions/dexSwapPairCalcFunctions",
      "@ouronet/ouronet-core/interactions/dexSwapPairDashboardFunctions",
      "@ouronet/ouronet-core/interactions/dexTrueFungibleFunctions",
      "@ouronet/ouronet-core/interactions/ouroAccountFunctions",
      "@ouronet/ouronet-core/interactions/ouroWrapFunctions",
      "@ouronet/ouronet-core/interactions/ouroPrimordialsFunctions",
      "@ouronet/ouronet-core/interactions/kadenaFunctions",
      "@ouronet/ouronet-core/interactions/errors",
    ];

    // Phase-4 reorg: the guide documents the WHOLE stack, but this repo only
    // ships the @stoachain half. The @ouronet/* subpaths cannot be resolved
    // here — importing them would mean depending on ouronet-libs, which is
    // exactly the direction the one-way dependency rule forbids. So each kind
    // of specifier gets the strongest check available from this side:
    //
    //   @stoachain/* -> really imported, as before.
    //   @ouronet/*   -> asserted to be CITED in the guide, which still locks
    //                   the doc against drift. Their runtime resolution is
    //                   covered by the matching test in ouronet-libs, where
    //                   those packages are actually installed.
    const isLocal = (s: string) => s.startsWith("@stoachain/");

    for (const subpath of subpaths.filter(isLocal)) {
    ...
    for (const subpath of subpaths.filter((s) => !isLocal(s))) {
      it.skip(`Subpath '${subpath}' resolves — covered in ouronet-libs, not resolvable here`, () => {
        // Intentionally not run in this repo. See the note above: resolving it
        // would require depending on ouronet-libs. Kept as a visible marker so
        // the guide's full subpath inventory stays readable from this side.
      });
    }
  });
  ...
  describe("Error classes cited in the doc resolve to constructors (LC-8-E)", () => {
    ...
    const errorCases: ErrorCase[] = [
      ...
      { name: "KadenaShapeError", subpath: "@ouronet/ouronet-core/interactions/errors" },
      ...
      { name: "CodexUnknownFieldError", subpath: "@ouronet/ouronet-core/codex" },
      { name: "UnknownSeedTypeError", subpath: "@ouronet/ouronet-core/codex" },
    ...
    // Same split as the subpath block above: construct what this repo ships,
    // doc-lock what lives in ouronet-libs.
    for (const { name, subpath } of errorCases.filter((c) => c.subpath.startsWith("@stoachain/"))) {
    ...
    for (const { name, subpath } of errorCases.filter((c) => !c.subpath.startsWith("@stoachain/"))) {
      it.skip(`${name} from ${subpath} — covered in ouronet-libs, not importable here`, () => {
        // Intentionally not run in this repo; see the note in the subpath block.
      });
    }
  });
